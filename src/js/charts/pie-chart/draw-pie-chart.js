...
var using = {
		...
		bar.beforeRender(function(data) {
			...
			if (isolated_data.length > 0) {
				...
				if (this.x.$scale == "time" && parsed.data.length > 0) {
					...
					if (groupRangeBand > pie_chart_config.maxColumnWidth) {
						bar.width(pie_chart_config.maxColumnWidth);
						// each group width needs to be reduced if we are enforcing max width
						barGroupWidth -= (groupRangeBand - pie_chart_config.maxColumnWidth);
					} else {
	...
	circles: function(circle, location) {
		...
		circle.beforeRender(function(data) {
			...
			var totalLineDotThresh = (totalLinePoints < pie_chart_config.lineDotsThresholdTotal);
